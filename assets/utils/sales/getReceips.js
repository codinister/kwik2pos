...
import { dmy, formatDate, ymd } from '../DateFormats.js';
import displayToast from '../displayToast.js';
import deleteAccessControl from './deleteAccessControl.js';
import sendReceiptWhatsapp from './customers/sendReceiptWhatsapp.js';
...
const getReceipts = async (allreceipts) => {
  if (allreceipts) {
    ...
    const arr = JSON.parse(sessionStorage.getItem('deletedreceipt'));
    if (arr) {
    ...
    document.addEventListener('change', (e) => {
      if (e.target.matches('.edit-receipt-inpt')) {
        ...
        const getreceipts = JSON.parse(sessionStorage.getItem('editreceipts'));
        const obj = { ...getreceipts, pay_id: id, [name]: value };
        sessionStorage.setItem('editreceipts', JSON.stringify(obj));
      }
    });
    ...
    document.addEventListener('click', (e) => {
      ...
      if (e.target.matches('.save-receipt-btn')) {
        ...
        const { pay_id, payment, receipt_date } = JSON.parse(
          sessionStorage.getItem('editreceipts')
        );
      ...
      if (e.target.matches('.deletepay')) {
        ...
        if (confirm('Are you sure you want to delete!')) {
          ...
          fetch(
            ...
            .then((resp) => resp.text())
            .then((data) => {
              if (!sessionStorage.getItem('deletedreceipt')) {
                sessionStorage.setItem('deletedreceipt', JSON.stringify([]));
              }
              const arr = JSON.parse(sessionStorage.getItem('deletedreceipt'));
              const newarr = [...arr, pay_id];
              sessionStorage.setItem('deletedreceipt', JSON.stringify(newarr));
            });
        } else {
        }
      }
      ...
      if (e.target.matches('.editpay')) {
        ...
        if (sessionStorage.getItem('editreceipts')) return;
